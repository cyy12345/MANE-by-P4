...
int main(int argc, char** argv)
{
	...
	uint8_t localip[]={10,0,0,3};
	RTPIPv4Address addr(localip,9000);
	//RTPIPv4Address addr1(localip,1212);
	status = session.AddDestination(addr);
	//status = session.AddDestination(addr1);
	if (status < 0)
	{
		std::cerr << RTPGetErrorString(status) << std::endl;
		exit(-1);
...
#define ddd
	...
	OpenBitstreamFile("../P4/Foreman/foreman.264");
	//OpenBitstreamFile("../jsvm-master/bin/foreman_1.264");//打开264文件，并将文件指针赋给bits,在此修改文件名实现打开别的264文件。
			...
			else if(n->len>MAX_RTP_PKT_LENGTH)
			{
				...
				while(t<=k)
				{
					...
					else if(k==t)//发送的是最后一个分片，注意最后一个分片的长度可能超过MAX_RTP_PKT_LENGTH字节（当l>1386时）。
					{
						...
						status = session.SendPacket((void *)sendbuf,l+1,96,false,0);
						if (status < 0)
						{
							std::cerr << RTPGetErrorString(status) << std::endl;
							exit(-1);
